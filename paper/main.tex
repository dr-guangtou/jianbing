% Version 1; preprint format
% 08/17/2021 16399 words
...
\setCJKmainfont[Scale=MatchUppercase]{FandolKai}
...
%% ---------------------------------------------------------------------------------------------- %%
%% Brief Discussion of ASAP model
%% ---------------------------------------------------------------------------------------------- %%
...
%% ---------------------------------------------------------------------------------------------- %%
%% Sigma_Mvir for richness-based clusters
%% ---------------------------------------------------------------------------------------------- %%
...
\subsection{Information Contained in the the Shape of \texorpdfstring{$\Delta\Sigma$}{DSigma}}
    ...
    %To further quantify this systematic difference, we design a summary statistic to compare the
    %\dsigma{} profiles of \menve{50}{100} and richness-selected samples. 
    %We integrate  \dsigma{} within two radial ranges
    %($0.2 < R \leq 0.8$ Mpc and $0.8 < R \leq 3.0$ Mpc) and calculate the ratio:
    ...
    %\noindent We then calculate the $S$ values of the observed \dsigma{} profile and its best--fit
    %model \dsigma{} profile, and use their ratio $S_{\rm obs}/S_{\rm mod}$ to quantify differences
    %in \dsigma{} profile \emph{shapes}.
    %In Figure \ref{fig:dsigma_summary}, we compare the $S_{\rm obs}$ of \redm{} and \camira{}
    %clusters to \menve{50}{100} sample in the left panel.
    %Both cluster profiles show considerably higher $S_{\rm obs}$ values than \menve{50}{100} (and
    %many other \mstar{}-based proxies) in Bin 1-2.
    %In the right panel, we show their $S_{\rm obs}/S_{\rm mod}$ ratios.
    %While the ratios are consistent with 1.0 in all four bins for \menve{50}{100}, both cluster 
    %samples display $>1.0$ ratios in Bin 1-3.
    %These results reinforce the conclusion that, unlike the \mstar{}-based \mvir{} proxies, 
    %richness-based clusters are not consistent with the ``pure scatter'' model.
    %Of course, any full-fledged analysis conducting posterior inference on cluster samples must
    %consider a host of other systematics and potential biases (\eg{} \citealt{Sunayama2020,
    %DES2020}), and as discussed further in \S\ \ref{sec:perfect_finder}, diagnostic summary
    %statistics such as $S_{\rm obs}/S_{\rm mod}$ can be employed to help quantify the impact of such
    %systematics as a complementary alternative to directly fitting for parameterization of the
    %effects (\eg{} \citealt{Melchior2017, Murata2018, McClintock2019}). 
...
%% ---------------------------------------------------------------------------------------------- %%
%% Discussion about the connection to the ASAP model.
%% ---------------------------------------------------------------------------------------------- %%
...
%% ---------------------------------------------------------------------------------------------- %%
%% Discussion about the scatter of stellar mass at fixed halo mass
%% ---------------------------------------------------------------------------------------------- %%
...
\subsection{Implications for Optical Cluster Finding}
    ...
    \update{
    In \citet{Zu2021a, Zu2021b}, the authors revealed a potentially interesting 
    correlation between the \mstar{} of the BCG at a given richness and the concentration of the dark matter halo.
    These results suggest that the \mstar{}-based cluster finders could also help
    provide more details about the assembly of massive dark matter halos.
    }
...
\section{Matching the lensing profiles}
    ...
    % Move here from the results section
...
%% --------------------------------------------------------------------------------------------- %%
%% Figure: Demonstration of the mock catalogue
%% ---------------------------------------------------------------------------------------------- %%
\begin{figure*}
  \centering
...
%% ---------------------------------------------------------------------------------------------- %%
%% Figure: Stellar mass defined by R50
%% ---------------------------------------------------------------------------------------------- %%
\begin{figure}
    \centering
