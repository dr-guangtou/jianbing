...
\section{Summary and Conclusions}
    ...
    survey, we exam a series of observables's capabilities as \mvir{} proxies for massive 
    ...
    In this work, we expplored a series of \mstar{} measurements including the \mstar{} 
    ...
    1) What is the \scatterMhaloObsSym{} level for each observable in the four bins.
    ...
    \song{Link back to each figure}
    
    \begin{itemize}

        \item \plan{Be careful with CModel, it could be worse than you thought.}

        \item \plan{
            Large aperture \mstar{} is better \mvir{} proxy, whose \dsigma{} profiles 
            match the simulation prediction very well. 
            And \mstar{} in larger aperture tends to be better \mvir{} proxy up until 
            $\sim 100$ kpc.
        }
        
        \item \plan{
            Interestingly, the \mstar{} in the outskirt such as the \menve{50}{100}
            is an even better \mvir{} tracer than aperture \mstar{}.
            It is possible that this is because the outskirt is dominated by the \exsitu{}
            component. And the \exsitu{} component has a much closer relationship to the 
            current \mvir{} than the \insitu{} component dominating the inner region.
        }
        
        \item \plan{
            Richness cluster finder such as \redm{} and \camira{} indeed show great performance
            in the cluster mass regime. But the outskirt \mstar{} is also very competitive.
            Once below \mvir{}$< 10^{14} M_{\odot}$, \menve{50}{100} is on a par with 
            richness finder and can even become the better one at very low richness regime.
            Compared to richness, \mstar{} of a single galaxy has several advantages that 
            make it worth exploring more.
        }

        \item \plan{
            Although the richness cluster finders show great \scatterObsSymMhalo{} values, 
            their \dsigma{} profiles can not be well-fit by our ``scatter''-only model, 
            suggesting other systematic issues are at play here.
        }

        \item \plan{
            Comparing the richness \dsigma{} profiles and the \menve{50}{100} or other \mstar{}-based 
            ones, we see systematic differences in their \dsigma{} profiles. [Describing the 
            differences briefly].
            Such difference could arise from a series of issues such as projection effect, 
            off-center effect, or assembly bias.
        }

        \item \plan{
            To apply \mstar{}-based proxies in the future, one needs to solve the satellite 
            problem. [Explain what we find.]
        }
    ...
    \todo{Move to the TopN intro section}
