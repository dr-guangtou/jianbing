...
\begin{abstract}
    ...
    \plan{
        ...
        halos with a \sigmh{} level comparable to the red-sequence cluster finders.
...
\section{Introduction}
    ...
    \begin{itemize}
        ...
        \item \plan{Intro of g-g lensing}
            ...
            %Unfortunately, except for a tiny fraction of extremely massive dark matter 
...
\section{Methodology and Modeling Framework}
    ...
    The we present our fiducial model based on \mdpl2{} and \smdpl{} simulations that are
...
\subsection{The Philosophy of \topn{} Test}
    ...
    \todo{Add simple explanation figure here}
    ...
    \begin{itemize}
        ...
        \item To apply this method in real data, we make use of the \dsigma{} profile based on 
            ...
            massive galaxies (\todo{check the number}; e.g., \citealt{Mandelbaum2006a}) to
            ...
            with the same sample from the \todo{XXX} \sqdeg{} SDSS survey (Leauthaud et al. in
            prep.).}.
        ...
        \item To quantitatively interpret the differences of \dsigma{} profiles, we 
            develop a toy model based on N-body simulations \mdpl2{} and \smdpl{} and assumes a
...
%% Model used in comparison
%% ---------------------------------------------------------------------------------------------- %%
\subsection{Estimate $\sigma(M_{\rm vir} | {\rm Obs})$ Values}
    \label{sec:dsigma_match}

    \todo{Placeholder: Under construction}

    \song{Check with Chris to see whether the following statement is true.}
    \plan{We then fix the slope of the best-fit model and only vary the \scatterObsSymMhalo{} value
        to achieve different \scatterMhaloObsSym{}...}

    \begin{itemize}
        
        \item \plan{How do we estimate \dsigma{} profile and its uncertainty in the simulation.
            How do we deal with the stochastic nature of this process?}

        \item \plan{If necessary, explain how do we combine the \mdpl2{} and \smdpl{} results.}

        \item \plan{Explain Figure \ref{fig:mdpl2}}.

        \item \plan{The matching process: how do we measure the \chisq{} values; how do we take
            the covariance matrix of the observed \dsigma{} profile into account.}
...
\section{Sample and Data Analysis}
    ...
    \aph{It's sort of disruptive to the flow of the paper to have this section in between \S\ref{sec:method} and \S\ref{sec:data}. Would it be viable to switch the order of these two sections?} \jul{I agree.}
...
\subsection{Galaxy-Galaxy Lensing Measurements}
    ...
    the \todo{Explain the photo-$z$ weight}.
...
\subsection{Richness-based proxies for Halo Mass}
    ...
    \aph{This subsection and the previous one are {\em really} long, and also contains lots of material that seems better suited for either the Discussion section, or an appendix. I think by the time we reach Section 4 of the paper, we don't want to be in a position where we are spending lots of text on the lit review.}
...
\subsubsection{\redm{} richness}
    ...
    \begin{itemize}
        ...
        \item \redm{} (red-sequence Matched-filter Probabilistic Percolation;
            \citealt{Rykoff2014, Rozo2014, Rozo2015a, Rozo2015b}, \addref{Rykoff2016})
        ...
        \item In this work, we use an internal version of \redm{} cluster catalog using
            HSC \texttt{S16A} catalog (\todo{XXX} private communication).
            ...
            \todo{Ask Eli Rykoff to say a few words about recent upgrades}.
            The catalog includes \todo{XXX} clusters between $0.1 < z <$\todo{XX}.
            ...
            $\sigma_{z}/(1 + z) \sim$\todo{XXX} for HSC.
            At $0.2 < z < 0.5$, we have \todo{XXX} clusters with
            $\lambda_{\rm redM, HSC} > 6$ after applying the bright star mask.

        \item \plan{There needs to be some discussion about the richness completeness
            within $0.2 < z < 0.5$.
            Figure 2 from McClintock et al. 2017}.

        \item \song{Do we want to mention this?}
            \plan{We also compare with the \dsigma{} profiles of $\lambda > 20$ clusters
            between DES and HSC...}
        ...
        \item We use the central candidate with the highest $P_{\rm cen}$ value as the
            ...
            fraction and off-center distance are \todo{XXX} (e.g., \addref{}).
            We have \todo{XXX} clusters (\todo{XXX\%}) with $P_{\rm cen} > 0.7$.
        ...
        \item In the same area, we also have \todo{XXX} clusters at $0.2 < z < 0.5$ from
...
\subsubsection{\camira{} richness}
    ...
    \begin{itemize}
        ...
        \item In this work, we use the \camira{} catalog based on the \texttt{S16A} data
            ...
            After applying the bright star mask, we use \todo{XXX} clusters with
            $0.2 < z < 0.5$ for our tests.
            The photo-$z$ accuracy of these clusters are $\sigma_{z}=$\todo{XXX}.
            ...
            measurements.
            \plan{Do these tests, and summarize the results here.}

        \item \plan{Discussion about the completeness of the cluster catalog.}
...
\subsection{Choice of Number Density Bins}
    ...
    \todo{Placeholder}
    \aph{Are we sure this warrants its own subsection?}
    ...
    \plan{Mention that the \mstar{} sample misses $\sim 10$\% of galaxies due to failed 1-D
        ...
        from the overall SMF. Some of these cases are on-going mergers or double-core
        BCGs. We do not correct for the difference in the effective volume caused by this.
        Varying the number density threshold by $\sim 10$\% accordingly does not change any
        key result.}
    
    \plan{The completeness of the richness--selected clusters should be highly complete in our
        redshift range for very massive clusters with \mvir{}$\gtrsim 10^{14} M_{\odot}$, but 
        can also suffer from $\sim 10$-20\% incompleteness in the low richness bins.
        We also do not attempt to correct for this effect.
        HSC images are more than deep enough to detect massive quiescent galaxies in these 
        clusters, so the incompleteness should not be caused by survey depth like in the 
        SDSS \redm{} catalog.
        These incompleteness can be caused by different photometric issues.
        It is also possible that richness-based cluster finders fail to pick up massive 
        halos with low richness of massive quiescent galaxies.
...
\section{Results}
    ...
    \aph{Maybe the rest of what follows can be relocated to a Caveats section at the end?} 
    We adopt strong
...
%% ---------------------------------------------------------------------------------------------- %%
  \begin{figure}
      ...
      \caption{
          ...
          effect at $R=$\todo{XX} Mpc at $M*=xx$, and a a maximum of 20\% effect at $R=$\todo{XX}
          Mpc at $M*=xx$.
          }
      \label{fig:satellite}
  \end{figure}
...
\subsection{Impact of Satellites}
    ...
    \todo{Update the XX numbers using the final model}
    ...
    scales is very low. Using HSC data we estimate the satellite fraction to be \todo{XX}\% while
    using he mock catalog we find the satellite fraction to be \todo{XX}\%. Within $11.7
    ...
    satellite fraction to be \todo{XX} in HSC data and \todo{XX} in the mock catalog. In the
    forth \mmax{} bin, the \mdpl2{} model predicts a more significant enhancement peaking at 20\%
...
%% ---------------------------------------------------------------------------------------------- %%
  \begin{figure*}
      ...
      \caption{
          \todo{Update the figure with correct \mvir{} distributions for the 3rd and 4th bins.}
          Comparison of the \topn{} test for \maper{100} and \mcmodel{}.
          ...
          is clear that the lensing amplitude for \maper{100} is significantly higher than for
          ...
          The second column is the ratio of the \dsigma{} profiles of \mcmodel{} and \maper{100}
          selected galaxies.
          \mcmodel{} based selections
          have lensing amplitudes that are $\sim$20-50\% lower than the \maper{100} based samples.
          The
...
\subsubsection{Performance of \mcmodel{}}
    ...
    \todo{Update the XX numbers using the fianl model}
    ...
    using our benchmark aperture stellar mass \maper{100}.
    The left panels of Figure \ref{fig:m100_cmod} show the \rdsigma{} profiles of the \mcmodel{}-
    and \maper{100}-selected samples.

    The \mcmodel{}-based samples clearly have much lower amplitudes in their \dsigma{} profiles
    compared to the \maper{100} ones in all four bins.
    Without the need of any modeling, we can immediately conclude that \maper{100} is a much
    ...
    Using the best-fit profiles for the \maper{100} selected samples as reference (gray shaded
    region), we can see that the \dsigma{} profile of the \maper{100} samples are well-described
    ...
    \todo{[XX, XX, XX, XX]} dex lower than the \maper{100}-selected ones.
    The best-fit \sigmh{} values are also much higher for the \mcmodel{} samples
    (\todo{[0.6, 0.6, 0.6, 0.7]} dex in the four bins) than the \maper{100} ones
    (\todo{[0.4, 0.4, 0.4, 0.4]}).
...
\subsubsection{Performance of the \mstar{} of outer envelope}
    ...
    \song{Update the XX numbers using the final model}
    ...
    \begin{itemize}
        ...
        \item According to our best-fit model, the \menve{50}{100}-selected samples have slightly
            higher mean \mvir{} (\logmvir{}=[\todo{XX, XX, XX, XX}]) in all four bins than the
            \maper{100}-selected samples (\logmvir{}=[\todo{XX, XX, XX, XX}]), while also
            displaying lower \sigmh{} values
            (\sigmh{}=[\todo{XX, XX, XX, XX}] for \menve{50}{100}; and
             \sigmh{}=[\todo{XX, XX, XX, XX}] for \maper{100}).
...
\subsubsection{Scatter of \mvir{} at Fixed Observables}
    ...
    \sigmh{} and the number density for three groups of different \mvir{} proxies explored in
    this work in Figure \ref{fig:scatter_trend}.
    We also record the ranges of observables in each bin and their \sigmh{} values in
    ...
    \begin{itemize}
        ...
        \item In the upper panel, we compare the representative \mvir{} proxies from several different
            ...
            the observable with lower \sigmh{} value in the same density bin a better massive halo
        ...
        \item As already highlighted in \S \ref{sec:cmodel}, \maper{100} is clearly a much better
            ...
            In addition, for all the \mstar{}-based \mvir{} proxies shown in this panel, their \sigmh{}
        ...
        \item To further demonstrate the second point, we compare the results for different
            ...
            Figure \ref{fig:scatter_trend}. For aperture \mstar{}, the \sigmh{} at fixed number
            density significantly decreases with increasingly large aperture value, especially in Bin
            1 \& 2. For Bin 1, \sigmh{} decreases from \todo{XX} for \maper{10 kpc}, and \todo{XX}
            for \maper{30}, to \todo{XX} for \maper{100} (green dashed line as reference). The
            \sigmh{} values for \maper{10} and \maper{30} do not decrease with volume number
            density as well.
            ...
            does not lead to unambiguous improvement of \sigmh{} value.
        ...
        \item In the top panel of Figure \ref{fig:scatter_trend}, we show that the \asap{} model
            also provides a reasonably good \mvir{} proxy whose \sigmh{} values
...
\subsection{Comparison with X--ray observation}
    ...
    \alexie{We are going to think about whether to add this here or not. Decide later.}
