...
\subsection{Philosophy of Top-N test}
    ...
    \begin{itemize}
        ...
        \item In observation, we reverse 
        
        \item Using shallow imaging surveys such as SDSS, one has to stack a large number of 
            massive galaxies (\todo{check the number}; e.g., \citealt{Mandelbaum2006a}) to achieve 
            a reasonable \snratio{}. 
            Now, the much deeper images with extraordinary seeing condition from the HSC survey 
            allows us to reach the same \snratio{} with much smaller lens sample size.
            For instance, the \dsigma{} profile of BOSS \texttt{LOWZ} galaxies at $0.31 < z < 0.43$
            from the $\sim 137$ \sqdeg{} HSC \texttt{S16A} region has comparable \snratio{}
            with the same sample from the \todo{XXX} \sqdeg{} SDSS survey (Leauthaud et al. in prep.).
            Such unprecedented g--g lensing capability allows to carefully 
